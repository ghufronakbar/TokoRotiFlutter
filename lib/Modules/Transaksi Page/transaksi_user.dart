...
class _TransaksiUserState extends State<TransaksiUser> {
  ...
  bool canManageTransactions = false;
  ...
  void fetchPreferences() async {
    // Ambil preferensi dari SharedPreferences
    final prefs = await SharedPreferences.getInstance();
    setState(() {
      canManageTransactions = prefs.getBool('can_manage_transactions') ?? false;
    });
  }
  ...
  void addToCart(dynamic product) {
    print("ProductName: ${product['nama_barang']}");
    print("aktif : ${product['aktif'].toString()}");

    if (product['aktif'] == 0) {
      // Tampilkan pesan error jika produk tidak aktif
      showDialog(
        context: context,
        builder: (context) => AlertDialog(
          title: Text('Produk Tidak Tersedia'),
          content: Text('Produk ini tidak tersedia untuk dijual.'),
          actions: [
            TextButton(
              onPressed: () {
